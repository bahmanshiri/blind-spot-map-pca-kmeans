# -*- coding: utf-8 -*-
"""
پایپ‌لاین دیتاست اصلی — بخش ۵.۳ مقاله (Table 2).

اجرا: بارگذاری/پاکسازی -> انکودینگ -> PCA(2) -> انتخاب k (Silhouette) ->
اعتبارسنجی خوشه‌ها با درخت‌تصمیم -> بازیابی گروه اقلیت (ARI).
"""
import os
import sys
import numpy as np
...
# 2) انکودینگ: عددی -> StandardScaler، gender -> ordinal، branch -> one-hot
...
# 4) انتخاب k بر اساس حداکثر Silhouette (روی نمونه‌ی زیرمجموعه برای سرعت)
...
k_business = 3  # طبق بخش ۵.۳: انتخاب نهایی برای تفسیرپذیری کسب‌وکار

# 5) خوشه‌بندی نهایی با k کسب‌وکاری (k=3) برای گزارش شکل ۳
...
# 6) اعتبارسنجی خوشه‌ها با درخت‌تصمیم (پیش‌بینیِ برچسبِ خوشه از روی ویژگی‌های خام)
...
# 7) بازیابی گروه اقلیت (anomalous) با k=2
...
print(f"\nذخیره شد: {DATA_DIR}/primary_dataset_clean.csv ({len(clean):,} ردیف)")
print(f"ذخیره شد: {RESULTS_DIR}/table2_primary_dataset_metrics.csv")
