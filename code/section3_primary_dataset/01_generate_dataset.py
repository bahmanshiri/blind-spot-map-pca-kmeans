# -*- coding: utf-8 -*-
"""
Primary dataset generator -- Section 3.1 of the paper.

120,000 synthetic bank customers with 3 hidden ground-truth segments:
  - normal (62%): ordinary customers
  - anomalous (5%): minority group with abnormal account turnover
  - low_activity (33%): low-transaction customers

Features:
  - numeric (4 features): age, account_balance, transaction_to_balance_ratio,
    avg_monthly_transactions
  - categorical: gender (binary, independent of segment), branch (160 branches,
    independent of segment)

Segment design:
  - normal: baseline values for all features
  - anomalous: transaction_to_balance_ratio strongly shifted (~21 standard
    deviations of the normal group on this same feature) -- this is the
    "abnormal account turnover" used in Section 4.3 as the "equivalent
    configuration" with ratio=5% and separation~21x
  - low_activity: avg_monthly_transactions much lower than normal
"""
import numpy as np
import pandas as pd
...
def generate_primary_dataset(n_total=120_000, seed=42, n_branches=160,
    ...
    # --- inject a number of exact duplicate records, overwriting random rows
    #     (total row count unchanged) -- simulates real raw-data artifacts ---
    ...
    # --- inject a number of extreme outliers in account_balance (data-entry error) ---
...
def clean_dataset(df):
    """Remove exact-duplicate records + extreme outliers (IQR on account_balance)."""
    before = len(df)
